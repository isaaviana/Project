...
namespace GalaxyCinemas.DataLayer
{
    public class DataLayer
    {
        ...
        private static GalaxyCinemas.DBtEST.GalaxyCinemaEntities databaseContext = new GalaxyCinemas.DBtEST.GalaxyCinemaEntities();
        ...
        {
            return databaseContext.Movies.Select(dbMovie => new Common.Movie { MovieID = dbMovie.MovieID, Title = dbMovie.Title }).ToList();
        ...
        public static List<Booking> GetBookingsInDateRange(DateTime fromDate, DateTime toDate)
        {
            ...
            var blabla = from booking in databaseContext.Bookings
                         from session in databaseContext.Sessions
                         from movie in databaseContext.Movies
                         where session.MovieID == movie.MovieID
                         && session.SessionDate >= fromDate.Date && session.SessionDate <= toDate.Date.AddDays(1)
                         select booking;
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                conn.Open();
                string sql = @"select BookingNumber, [Booking].SessionID, Quantity, Special, OriginalPrice, Discount, FinalPrice, [Session].SessionDate from [Booking] inner join [Session] on [Booking].SessionID = [Session].SessionID where [Session].SessionDate >= @startdate and [Session].SessionDate < @enddate";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
        ...
        public static void AddBooking(Booking booking)
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    ...
                    {
                        specialParam.Value = booking.Special;
        ...
        public static List<Session> GetAllSessionsForMovie(int movieID, DateTime date)
        {
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session] where MovieID = @movieID and SessionDate >= @startDate and SessionDate < @endDate";
                using (SqlCommand command = new SqlCommand(sql, conn))
                {
        ...
        {
            Session allSession = new Session();
            List<Session> sessions = new List<Session>();
            ...
            {
                conn.Open();
                string sql = @"select SessionID, MovieID, SessionDate, CinemaNumber from [Session] where MovieID = @movieID and SessionDate = @thisDate";

                using (SqlCommand command = new SqlCommand(sql, conn))
                {
                    command.Parameters.AddWithValue("movieID", allSession.MovieID);
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            session.SessionID = reader.GetInt32(0);
                            session.MovieID = reader.GetInt32(1);
                            session.SessionDate = reader.GetDateTime(3);
                            sessions.Add(session);
                        }
        ...
        /// <summary>
        /// Get list of all Movies from the database based on the sessionID.
        /// </summary>
